# CityInsight/Amsterdam_Roads_networks.py
import osmnx as ox
import geopandas as gpd
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches

def plot_amsterdam_roads_network(show_plot=True):
    # Define the place and feature types
    place_name = "Amsterdam, Netherlands"

    # Get the boundary of Amsterdam as a polygon
    amsterdam_polygon = ox.geocode_to_gdf(place_name).geometry.iloc[0]

    # Get parks data from OSM using the polygon
    parks = ox.features_from_polygon(amsterdam_polygon, tags={'leisure': 'park'})

    # Get libraries data from OSM using the polygon
    libraries = ox.features_from_polygon(amsterdam_polygon, tags={'amenity': 'library'})

    # Get museums data from OSM using the polygon
    museums = ox.features_from_polygon(amsterdam_polygon, tags={'tourism': 'museum'})

    # Get road network data from OSM
    roads = ox.graph.graph_from_polygon(amsterdam_polygon, network_type='all')
    ...
    ax.set_title("City of Amsterdam\nParks, Libraries, Museums, and Roads", fontsize=20, weight='bold', loc='left')

    # Remove axes for a clean look
    ax.set_axis_off()

    # Save the plot as PNG with a white background
    plt.savefig('amsterdam_roads_parks_libraries_museums.png', dpi=300, bbox_inches='tight', facecolor='white', edgecolor='white')
